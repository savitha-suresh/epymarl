...
class TransformerAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.inference_net = nn.Sequential(nn.Linear(args.hidden_dim + input_shape, NN_HIDDEN_SIZE),
                                           nn.BatchNorm1d(NN_HIDDEN_SIZE),
                                           activation_func,
                                           nn.Linear(NN_HIDDEN_SIZE, args.latent_dim * 2))

        self.latent = torch.rand(args.n_agents, args.latent_dim * 2)  # (n,mu+var)
        self.latent_infer = torch.rand(args.n_agents, args.latent_dim * 2)  # (n,mu+var)
        ...
        self.fc2_w_nn = nn.Linear(NN_HIDDEN_SIZE, args.hidden_dim * args.n_actions)
        ...
        self.mi= torch.rand(args.n_agents*args.n_agents)
        self.dissimilarity = torch.rand(args.n_agents*args.n_agents)
    ...
    def init_latent(self, bs):
        ...
        #mask = 1 - torch.eye(self.n_agents).byte()
