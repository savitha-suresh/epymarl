...
class BasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False, t_end=None, actions=None, return_aux_losses=False):
        ...
        if return_aux_losses:
            agent_outs, hidden_states, losses, context_out = self.agent(agent_inputs, memory=memory, attn_mask=mask, actions=actions, 
                                                       return_aux_losses=return_aux_losses, timestep=t)
        else:
            agent_outs, hidden_states, context_out = self.agent(agent_inputs, memory=memory, attn_mask=mask, actions=actions,
                                                   return_aux_losses=return_aux_losses, timestep=t)
        
        
        context_out = context_out.expand(ep_batch.batch_size, -1)
        ep_batch["context"][:, t] = context_out.detach()
